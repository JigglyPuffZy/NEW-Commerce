...
const AllOrdersPage = () => {
  ...
  useEffect(() => {
    if (sellerId) {
      fetch(`https://rancho-agripino.com/database/potteryFiles/fetch_accepted_orders.php?sellerId=${sellerId}`)
  ...
  const handleApprove = () => {
    fetch('https://rancho-agripino.com/database/potteryFiles/update_order_status_toship.php', {
        method: 'POST',
        headers: {
        ...
        .then((data) => {
          ...
          if (data.success) {
            ...
            Alert.alert('Success! Order is to ship.'); // Alert here
          } else {
  ...
  const handleOrderClick = (order) => {
    if (order.order_status === '1') { // Check for Pending status
...
{item.product_name && item.imagesPath && (
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
            data={orders.filter((order) => order.order_status === '1')} 
      ...
      <Modal visible={isModalVisible} animationType="slide" transparent={true}>
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                <Text style={styles.modalText}>Is the order to ship?</Text>
